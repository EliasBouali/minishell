	...
	while (i < n)
	{
		if (buf[i] == '\n' || buf[i] == '\r')
		{
			if (buf[i] == '\r' && (i + 1) < n && buf[i + 1] == '\n')
				i++;
			if (line_is_delim(line_buffer->buf, line_buffer->len, delim))
				return (1);
			write_line_and_reset(line_buffer, write_file_descriptor);
			if (buf[i] == '\r')
		...
		{
			if (line_buffer_push_char(line_buffer, buf[i]) == -1)
				return (-1);
		}
		i++;
	}
